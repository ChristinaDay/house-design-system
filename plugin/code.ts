...
async function createAccordion(): Promise<void> {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    titleText.x = spacing[4] as number;
    ...
    if (i === 0) {
      ...
      contentText.x = spacing[4] as number;
      contentText.y = spacing[2] as number;
...
async function createDialog(): Promise<void> {
  ...
  description.x = spacing[6] as number;
  ...
  buttonGroup.x = spacing[6] as number;
  ...
  buttonGroup.itemSpacing = spacing[3] as number;
...
async function createTabs(): Promise<void> {
  ...
  contentTitle.x = spacing[6] as number;
  contentTitle.y = spacing[6] as number;
  ...
  contentText.x = spacing[6] as number;
  contentText.y = spacing[6] as number + 40;
...
async function createDropdown(): Promise<void> {
  ...
  triggerText.x = spacing[3] as number;
  ...
  for (let i = 0; i < menuItems.length; i++) {
    ...
    itemText.x = spacing[3] as number;
...
async function createProgressBar(): Promise<void> {
...
async function createSwitch(): Promise<void> {
...
async function createColorPalette(): Promise<void> {
  ...
  let currentY = spacing[6] as number;
  const leftMargin = spacing[6] as number;
  const swatchSize = 60;
  ...
  for (const family of colorFamilies) {
    ...
    for (const colorKey of swatchKeys) {
      const colorValue = (family.colors as any)[colorKey];
  ...
  palette.resize(1200, currentY + (spacing[6] as number));
...
async function createEditorialHeadline(): Promise<void> {
...
async function createSystemDisplay(): Promise<void> {
  ...
  try {
    ...
    systemText.x = spacing[6] as number;
    systemText.y = spacing[6] as number;
    ...
    displayText.x = spacing[6] as number;
    displayText.y = (spacing[6] as number) + 30; // Position below SYSTEM
